...
import {deleteContact, updateFavourite} from '../Database/Db';

let db = openDatabase({name: 'ContactDatabase.db'});

export default function FavouriteContactList({navigation}) {
  ...
  const getFavouriteContacts = () => {
    db.transaction(txn => {
      txn.executeSql(
        'SELECT * FROM contacts WHERE favourite="1"',
        [],
        (sqlTxn, res) => {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>Favourite Contact Lists</Text>
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#f2f2f2'}}>
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>Favorite Contact Lists</Text>
        </View>
